...
class Exporter implements ExporterInterface {
   ...
   */
  public function exportEntityTypeContent($entity_type_id, $bundles = NULL) {
    $definition = $this->entityTypeManager->getDefinition($entity_type_id);
    if (!is_a($definition->getClass(), ContentEntityInterface::class, TRUE)) {
      throw new \InvalidArgumentException(sprintf('Entity type "%s" isn\'t a content entity', $entity_type_id));
    }
    ...
    if ($bundles) {
      ...
      if (!$bundle_key) {
        throw new \InvalidArgumentException(sprintf('Entity "%s" doesn\'t have a bundle key, but bundles were specified.', $entity_type_id));
      }
      $conditions[$bundle_key] = $bundles;
    }
